Shader "Custom/SpriteCircleFilled"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        CGPROGRAM
            ...
            fixed4 MySpriteFrag(v2f IN) : SV_Target
            {
                ...
                fixed yAngle = GetAngle(fixed2(0,1), IN.texcoord - uvCenter);
                fixed xAngle = GetAngle(fixed2(1,0), IN.texcoord - uvCenter);
                if (xAngle > 90) yAngle = 360 - yAngle;
                c.a *= (yAngle <= _Angle) * distance(IN.texcoord, uvCenter)*2 > _NearClip;
                c.rgb *= c.a;
                return c;
            }
        ENDCG
